...
import { getProductDeeplink, site } from "@/lib/site-config";

const FALLBACK_MS = 2500;

type Props = { productId: string };

export function ProductDeepLinkClient({ productId }: Props) {
  const [phase, setPhase] = useState<"opening" | "fallback">("opening");

  useEffect(() => {
    window.location.href = getProductDeeplink(productId);
